...
const AdminPanel: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.featured && !products[editingIndex]?.featured) {
      if (featuredCount >= 3) {
        showMessage(
          "You can only feature up to 3 products in the Hero section.",
          "error"
        );
        return;
      }
  ...
  const toggleFeatured = async (index: number) => {
    ...
    try {
      ...
      if (updatedProducts.length === 0) {
        showMessage(
          "Error: Products array is empty. Aborting update.",
          "error"
        );
        return;
      }
  ...
  const updateBannerSettings = async (newSettings: BannerSettings) => {
    try {
      ...
      const result: ApiResponse = await response.json();
      if (!result.success)
        throw new Error(result.error || "Failed to update banner settings");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between min-h-16 max-h-fit">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span className="text-sm sm:text-lg font-medium">Back to Store</span>
              </button>
            </div>
            <div className="flex items-center space-x-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500">
                Control your site's promotional banner
              </p>
            </div>
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleBannerChange}
                  className="w-full px-3 py-2 border text-black border-gray-300 rounded-lg focus:ring-2 "
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      ...
                      {products.length > 0
                        ? (
                            products.reduce(
                              (sum, p) =>
                                sum + parseFloat(String(p.price ?? "0")),
                              0
                            ) / products.length
                          ).toFixed(2)
                        : "0.00"}
    ...
    <div className="px-4 py-3 sm:px-6 sm:py-4 bg-gray-50 border-b border-gray-200">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <button
          ...
          className="inline-flex items-center justify-center px-3 py-1.5 border border-gray-300 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors w-full sm:w-auto"
        >
          <RefreshCw
